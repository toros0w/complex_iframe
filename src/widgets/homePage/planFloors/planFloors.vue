  ...
  <div class="planFloors">
    ...
    >
      <template #footer>
        ...
        >
          <Button @click.stop="deleteFloorPlan" severity="danger">
            <svg
              width="18"
              height="18"
              viewBox="0 0 18 18"
    ...
    <Dialog v-model:visible="showEditSideWindow" modal header="Задать стороны света" :style="{ width: '50vw' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
      <PlanEditWindowSideWorld
        v-if="formData"
        :images="data.selectedFloorsPlans.map(plan => ({url: plan.image, name: plan.name}))"
        :formData="formData"
      />
      <template #footer>
        <div class="btns w-full flex justify-between">
          <Button @click.stop="updatePlans(data.selectedFloorsPlans.map((plan) => plan.id), formData)" severity="success">
          ...
          <Button severity="secondary" @click="showEditSideWindow = false">
            Отмена
          </Button>
        </div>
      </template>
    ...
    <div class="planRooms__btns flex items-center gap-x-7.5 my-7.5">
      ...
      <Button @click.stop="onAddPlansClick" severity="success">
        <svg
          width="18"
          height="18"
          viewBox="0 0 18 18"
      ...
      <Button
        ...
        class="ml-auto"
        severity="info"
        :disabled="!data.selectedFloorsPlans.length"
      >
        <template #default>
          Действия
          <svg
    ...
    <PlanFloorTable :data="data" :plans="plans"/>
  </div>
</template>
...
import PlanEditWindowSideWorld from "../planLayout/planEditWindow/planEditWindow-sideWorld.vue";
...
const data = ref({ selectedFloorsPlans: [] });
...
const showEditSideWindow = ref(false);
const urls = ref("");
...
const menuActionsItems = ref([
  {
    ...
    command: () => {
      showEditSideWindow.value = true;
    },
  },
  {
    label: "Отключить отображение сторон света",
    command: () => {
      updatePlans(data.value.selectedFloorsPlans.map(plan => plan.id), {has_light_angle: false}).then(() => {
        isVisibleOffSide.value = true;
      })
    },
  },
  {
...
const updatePlans = async (plan_ids , data) => {
  return api.updateFloorPlans(plan_ids, [], data, true)
    .then((response) => {
      if (response.data && Array.isArray(response.data)) {
        showEditSideWindow.value = false
        plans.value.forEach((currPlan) => {
          if (plan_ids.includes(currPlan.id)) {
...
const getPlans = () => {
  return api.getFloorPlans(props.house.id)
    .then((response) => {
      if (response.data && Array.isArray(response.data)) {
...
const deleteFloorPlan = () => {
  const deletedFloorPlansIds = [];
  const promises = data.value.selectedFloorsPlans.map((floor_plan) => {
    deletedFloorPlansIds.push(floor_plan.id)
    return api.deleteFloorPlan(floor_plan.id)
  })

  Promise.all(promises).then(() => {
    plans.value = plans.value.filter((plan) => !deletedFloorPlansIds.includes(plan.id))
    isVisibleDeleteAll.value = false
  })
}
