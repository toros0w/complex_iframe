<template>
  <DataTable
    v-model:selection="data.selectedFloorsPlans"
    :value="plans"
    dataKey="id"
  ...
  >
    ...
    >
      <template #body="slotProps">
        ...
        <OverlayPanel v-show="hoveredImage" :visible="true" ref="op" :pt="overlayPanelStyle">
          <img
    ...
    <Column header="Имя файла" style="width: 20%" :pt="columnStyle">
      <template #body="slotProps">
        <div v-html="slotProps.data.name" class="text-center"></div>
      </template>
    </Column>
    <Column header="Подъезды" style="width: 5%" :pt="columnStyle">
      <template #body="slotProps">
        <div class="flex flex-col justify-center items-center">
          <div
            v-html="slotProps.data.entrances?.map((entrance) => entrance.position)"
            class="text-center text-grey-900"
          ></div>
        </div>
      </template>
    </Column>
    <Column field="quantity" header="Этажи" style="width: 5%" :pt="columnStyle">
      <template #body="slotProps">
        <p
          class="text-center text-grey-900"
          v-html="slotProps.data.floors?.map((floor) => floor.position)"
        />
      </template>
      <!-- slotProps.data.floor_positions?.length ? slotProps.data.floor_positions.reduce((acc, curr) => acc += ` ${curr},`, '') : 'Не установлено' -->
    </Column>
    <Column
      field="quantity"
    ...
    <Column header="Действия" style="width: 30%" :pt="columnStyle">
      <template #body="slotProps">
        <div class="flex 2xl:flex-row xl:flex-col items-center">
          <Button
            severity="success"
            class="2xl:mr-2.5 mb-2.5 xl:w-full flex justify-center"
            @click="router.push(`floors/${slotProps.data.id}`)"
          >
            <svg
              width="19"
              height="18"
            ...
            Произвести разметку этажа
          </Button>
          <Button
            severity="danger"
            class="xl:w-full flex justify-center h-max"
            @click="showTemplate($event, slotProps.data.id, index)"
          >
            <svg
              width="19"
              height="18"
...
const props = defineProps({ plans: Array, data: Object })
...
const columnStyle = ref({
  headerCell: {
    class: [
      "!bg-white !border-t-0 !border-grey-900 first:!border-l-0 last:!border-r-0 !py-2.5 !px-0",
    ],
  },
  bodyCell: {
...
const showTemplate = (event, floor_plan_id, index) => {
  confirm.require({
    ...
    accept: () => {
      deleteFloorPlan(floor_plan_id).then(() => {
...
const floorPositions = computed(() => (
  props.data.floors.map((floor) => floor.position)
))

const deleteFloorPlan = (floor_plan_id) => {
  return api.deleteFloorPlan(floor_plan_id)
}
